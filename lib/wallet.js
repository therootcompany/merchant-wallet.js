...
exports.Wallet = {};
exports.Wallet.create = function (myCoin, direction = 0, account = 0) {
  ...
  let derivationPath = `m/44'/${myCoin.coinType}'/${account}'/${direction}`;
  //console.error(derivationPath);
  ...
  // Private to Private
  wallet.wifFromMnemonic = async function (mnemonic, index, direction = 0, account = 0) {
    mustMnemonic(mnemonic);
    let seed = await Bip39.mnemonicToSeed(mnemonic);
    let privateRoot = HdKey.fromMasterSeed(seed);
    let wif = await wallet.wifFromPrivateRoot(privateRoot, index, direction, account);

    return wif;
  };
  wallet.wifFromPrivateRoot = async function (privateRoot, index, direction = 0, account = 0) {
    //mustMnemonic(mnemonic);
    //let seed = await Bip39.mnemonicToSeed(mnemonic);
    //let privateRoot = HdKey.fromMasterSeed(seed);
    let childKey = privateRoot.derive(
      `m/44'/${myCoin.coinType}'/${account}'/${direction}/${index}`
    );
  ...
  wallet.wifQrFromMnemonic = async function (mnemonic, index, opts, direction, account) {
    mustMnemonic(mnemonic);
    let seed = await Bip39.mnemonicToSeed(mnemonic);
    let privateRoot = HdKey.fromMasterSeed(seed);
    let wif = await wallet.wifFromPrivateRoot(privateRoot, index, direction, account);
  ...
  wallet.addrFromPrivateRoot = async function (
    privateRoot,
    index,
    direction = 0,
    account = 0
  ...
  wallet.qrFromAddr = function (addr, amount, opts) {
    let content = addr;
    if ("dash" === myCoin.name && addr.length <= 36) {
      let search = "";
      if (amount) {
    ...
    if ("ascii" === opts?.format) {
      return Qr.ascii(content, { indent: opts.indent ?? 4 });
    }
